  ...
  return (
    <article className="blueprint" style={{ display: 'flex', flexDirection: 'column', background: 'transparent' }}>
      ...
      <div onClick={() => nav(`/courses/${c.slug}`)} style={{ aspectRatio: '16/10', background: c.cover_url ? `center/cover no-repeat url(${c.cover_url})` : 'var(--color-accent-100)', borderBottom: '1px solid var(--color-divider)', display: 'grid', placeItems: 'center', position: 'relative', cursor: 'pointer' }}>
        ...
        {onSale && <span style={{ position: 'absolute', top: 12, right: 12, fontFamily: 'var(--font-heading)', fontSize: 12, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', color: '#fff', background: '#B42318', padding: '3px 10px' }}>Sale</span>}
      ...
      <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginTop: 4, paddingTop: 14, borderTop: '1px solid var(--color-divider)' }}>
          <span style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ fontFamily: 'var(--font-heading)', fontSize: 24, color: onSale ? '#B42318' : undefined }}>{c.price_cents ? money(onSale ? (c.sale_price_cents as number) : c.price_cents, c.currency) : 'Free'}</span>
            {onSale && <span style={{ fontSize: 15, color: 'var(--color-neutral-600)', textDecoration: 'line-through' }}>{money(c.price_cents, c.currency)}</span>}
          </span>
