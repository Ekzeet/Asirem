...
export default function CourseBuilder() {
  ...
  const { data, loading, reload } = useAsync(async () => {
    const [{ data: course }, { data: sections }, { data: quizzes }, { data: assignments }] = await Promise.all([
      supabase.from('courses').select('id,title,status,category,accent,icon,cover_url,subtitle,description,level,price_cents,instructor_id,drip_enabled,credit_hours,slug').eq('id', courseId!).single(),
