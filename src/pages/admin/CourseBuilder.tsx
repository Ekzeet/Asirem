...
export default function CourseBuilder() {
  ...
  const { data, loading, reload } = useAsync(async () => {
    const [{ data: course }, { data: sections }, { data: quizzes }, { data: assignments }] = await Promise.all([
      supabase.from('courses').select('id,title,status,category,accent,icon,cover_url,subtitle,description,level,price_cents,sale_price_cents,sale_starts_at,sale_ends_at,instructor_request_price_cents,instructor_id,drip_enabled,credit_hours,slug,is_live,zoom_url,zoom_host_email,module_lock').eq('id', courseId!).single(),
