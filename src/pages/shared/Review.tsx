...
export default function Review() {
  ...
  const { data, loading, reload } = useAsync(async () => {
    const { data: subs } = await supabase
      ...
      .select('id, body, link_url, status, grade, feedback, submitted_at, student:profiles!submissions_student_profile_fkey(full_name), assignment:assignments(title, points, course:courses(title))')
      .order('submitted_at', { ascending: false })
    const rows: Submission[] = (subs ?? []).map((s: any) => ({
      id: s.id, body: s.body, link_url: s.link_url, status: s.status, grade: s.grade, feedback: s.feedback,
...
function GradeModal({ sub, onClose, onSaved }: { sub: Submission; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    setBusy(true)
    await supabase.from('assignment_submissions').update({ grade, feedback, status: 'graded', graded_by: me!.userId, graded_at: new Date().toISOString() }).eq('id', sub.id)
    setBusy(false); onSaved()
  }

  return (
    <Modal title={t('gradeSubmission')} subtitle={`${sub.student} · ${sub.assignmentTitle}`} onClose={onClose} width={560}
      footer={<><BtnGhost onClick={onClose}>{t('cancel')}</BtnGhost><BtnPrimary onClick={save} disabled={busy}><Icon name="check" size={16} />{t('saveGrade')}</BtnPrimary></>}>
      <div style={{ background: '#F7F9FC', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px', marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 800, color: '#8494A8', textTransform: 'uppercase', letterSpacing: .4, marginBottom: 6 }}>{t('submission')}</div>
